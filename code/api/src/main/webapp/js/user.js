...
if (userTokenData) {
  ...
  fetch(`https://api.berryscan.tech/api/user/${userTokenData.userId}`, requestOptions)
...
function saveChanges() {
  ...
  if (userTokenData) {
    ...
    // Fetch user data from the server
    fetch(`https://api.berryscan.tech/api/user/update`, requestOptions)
        .then(response => {
          if (!response.ok) {
...
function logout() {
    ...
    fetch("https://api.berryscan.tech/api/logout/"+userTokenData.userId, requestOptions)
        .then(response => {
            if (!response.ok) {
                throw new Error(`HTTP error! Status: ${response.status}`);
            }
